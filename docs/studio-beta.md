...
- 本地模型页只显示可直接管理的 Qwen3 ASR 0.6B／1.7B；共享 Forced Aligner 仅在下载、校验、失败或缺失时内联显示。Silero VAD 是随应用提供的运行时，旧 WeSpeaker 中文权重仅保留为兼容回退，两者不作为模型卡片。
- 说话人运行时会优先选择已校验的多语言权重，缺少时才回退已有的旧中文权重。多语言 VoxBlink2 权重只有在官方归档的固定大小与 SHA-256 都被核实后才会开放下载；不会用未校验的链接替代。
...
- 诊断页只检查 App 内置的 ffmpeg/ffprobe、libass、H.264/AAC 编码能力、ASR/Speaker 运行时、模型依赖链和剩余磁盘；不会回退到 PATH、Homebrew 或系统 Python。缺失项会显示为阻断状态，不会伪装成可用。
