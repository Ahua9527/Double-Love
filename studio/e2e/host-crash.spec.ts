...
async function invoke<T>(page: Page, name: string, payload?: unknown): Promise<T> {
  const response = await page.evaluate(
    async ({ command, commandPayload }) => window.doubleLove.invoke(command, commandPayload),
    { command: name, commandPayload: payload },
  ) as { status: string; result?: { type: string; data?: T }; error?: { code: string } }
  if (response.status !== 'ok' || response.result?.type !== 'invoke') return response as unknown as T
...
function hostPidFor(appPid: number): number {
  ...
  const listing = spawnSync('ps', ['-eo', 'pid,ppid,comm'], { encoding: 'utf8' })
  const line = listing.stdout
    .split('\n')
    .find((entry) => entry.includes(hostBinary) && Number(entry.trim().split(/\s+/)[1]) === appPid)
...
test.beforeAll(() => {
  ...
  temporaryRoot = mkdtempSync(join(tmpdir(), 'double-love-electron-crash-e2e-'))
...
test('a killed host rejects calls, marks the crash, and never replays writes', async () => {
  ...
  execFileSync(process.env.DOUBLELOVE_FFMPEG || 'ffmpeg', [
    ...
    '-f', 'lavfi', '-i', 'color=c=black:s=320x180:r=25:d=61',
    '-f', 'lavfi', '-i', 'sine=frequency=440:sample_rate=48000:duration=61',
    '-c:v', 'mpeg4', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-shortest',
    mediaPath,
  ])

  const first = await launch(userData)
  const createGrant = await first.page.evaluate(
    async (e2ePath) => window.doubleLove.dialogs.pickDirectory({ title: 'crash project', kind: 'project-open', e2ePath }),
    projectRoot,
  )
  const created = await invoke<{ status: string }>(first.page, 'project_create', { grantToken: createGrant?.token })
  ...
  const imported = await invoke<{ status: string; data?: { id: string } }>(first.page, 'import_media', { grantToken: mediaGrant?.token })
  expect(imported.status).toBe('success')
  const assetId = imported.data?.id as string

  const started = await invoke<{ status: string; data?: { task_id: string } }>(first.page, 'transcribe_start', { assetId, model: 'qwen3-asr-0.6b', language: 'auto' })
  ...
  await expect.poll(async () => {
    const response = await first.page.evaluate(() =>
      window.doubleLove.invoke('assets_list', {})
        .catch((error: unknown) => ({ ipcError: error instanceof Error ? error.message : 'unknown' })))
  ...
  await expect.poll(() => existsSync(crashMarker), { timeout: 10_000 }).toBe(true)
  const marker = JSON.parse(readFileSync(crashMarker, 'utf8')) as Record<string, unknown>
  ...
  const reopened = await invoke<{ status: string }>(second.page, 'project_open', { grantToken: reopenGrant?.token })
  expect(reopened.status).toBe('success')
  const transcript = await invoke<{ status: string; data?: { words: unknown[] } }>(second.page, 'transcript_get', { assetId })
...
function mkdirSyncForModels(userData: string): void {
  ...
  writeFileSync(join(modelRoot, 'installations.json'), `${JSON.stringify(installations, null, 2)}\n`)
}
