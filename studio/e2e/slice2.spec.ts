...
async function createProject(path: string): Promise<InvokeOperation<ProjectSummary>> {
  return invoke<ProjectSummary>('project_create', { grantToken: await directoryGrant(path) })
}

async function openProject(path: string): Promise<InvokeOperation<ProjectSummary>> {
  return invoke<ProjectSummary>('project_open', { grantToken: await directoryGrant(path) })
}

function cliOperation(args: string[]): InvokeOperation<Record<string, unknown>> {
  return JSON.parse(execFileSync(cliBinary, ['--json', '--project', projectRoot, ...args], {
    encoding: 'utf8',
...
function addMainTrackClipWithCli(): void {
  ...
  const appended = cliOperation(['main-track-add', '--asset', assetId as string])
...
test.beforeAll(async () => {
  ...
  temporaryRoot = mkdtempSync(join(tmpdir(), 'double-love-electron-slice2-e2e-'))
...
test('creates, reopens, navigates history, restores, and persists project identity', async () => {
  ...
  const initialHistory = await invoke<Array<{ revision: number; operation: string; restorable: boolean }>>('project_history')
  ...
  const persistedDefaultStyle = await invoke<Record<string, unknown>>('subtitle_style_get')
  ...
  const styleChanged = await invoke<Record<string, unknown>>('subtitle_style_set', { style: changedStyle })
  ...
  const styleHistory = await invoke<Array<{ revision: number; operation: string; restorable: boolean }>>('project_history', { limit: 1 })
  expect(styleHistory.data).toEqual([
    expect.objectContaining({ revision: 2, operation: 'subtitle_style_set', restorable: true }),
  ])

  const recent = await invoke<Array<{ project_id: string; root: string; exists: boolean }>>('recent_projects_list')
  expect(recent.status).toBe('success')
  expect(recent.data).toHaveLength(1)
  expect(recent.data?.[0]).toMatchObject({ project_id: projectId, exists: true })
  ...
  const changed = await invoke<typeof changedCanvas>('canvas_set', { canvas: changedCanvas })
  expect(changed).toMatchObject({ status: 'success', revision: 3, data: changedCanvas })
  expect((await invoke<number>('project_revision')).data).toBe(3)

  const history = await invoke<Array<{ revision: number; operation: string; restorable: boolean }>>('project_history', { limit: 10 })
  expect(history.data).toEqual(expect.arrayContaining([
    expect.objectContaining({ revision: 3, operation: 'canvas_set', restorable: true }),
  ]))

  const undo = await invoke<null>('edit_undo')
  expect(undo).toMatchObject({ status: 'success', revision: 4 })
  expect((await invoke<{ width: number }>('canvas_get')).data?.width).toBe(1920)

  const redo = await invoke<null>('edit_redo')
  expect(redo).toMatchObject({ status: 'success', revision: 5 })
  expect((await invoke<{ width: number }>('canvas_get')).data?.width).toBe(1280)

  addMainTrackClipWithCli()
  expect((await invoke<number>('project_revision')).data).toBe(6)
  const historyAfterCli = await invoke<Array<{ revision: number; operation: string; restorable: boolean }>>('project_history', { limit: 10 })
  expect(historyAfterCli.data).toEqual(expect.arrayContaining([
    expect.objectContaining({ revision: 3, operation: 'canvas_set', restorable: true }),
    expect.objectContaining({ revision: 6, operation: 'main_track_append', restorable: true }),
  ]))

  const restored = await invoke<{ restored_revision: number; revision: number }>('project_restore_revision', { revision: 1 })
