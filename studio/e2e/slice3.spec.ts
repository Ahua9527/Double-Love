...
test.beforeAll(async () => {
  ...
  temporaryRoot = mkdtempSync(join(tmpdir(), 'double-love-electron-slice3-e2e-'))
...
test('imports, lists, and serves a real project asset through dl-media', async () => {
  ...
  expect(listed.data).toEqual([expect.objectContaining({ id: assetId, status: 'prepared' })])
  ...
  const rejectedUrls = await electronApp.evaluate(async ({ net }, sourcePath) => {
    const urls = [
  ...
  const rendererResolve = await page.evaluate(() =>
    window.doubleLove.invoke('resolve_media_asset', { asset_id: 'probe' })) as InvokeEnvelope<unknown>
  expect(rendererResolve).toMatchObject({ status: 'error', error: { code: 'IPC_FORBIDDEN' } })
  ...
  expect(JSON.stringify(probeFailure)).not.toContain(realpathSync(probeFailureMediaPath))
  ...
  expect((await invoke<MediaAssetSummary[]>('assets_list')).data).toHaveLength(1)
})
