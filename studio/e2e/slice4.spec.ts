...
function generateMedia(path: string, color: string, frequency: number): void {
  execFileSync(process.env.DOUBLELOVE_FFMPEG || "ffmpeg", [
    ...
    "lavfi",
    "-i",
    `color=c=${color}:s=320x180:r=25:d=1`,
    "-f",
    "lavfi",
    "-i",
...
test.beforeAll(async () => {
  ...
  temporaryRoot = mkdtempSync(
    join(tmpdir(), "double-love-electron-slice4-e2e-"),
  );
...
test("edits the main track and round-trips project visual settings without path exposure", async () => {
  ...
  expect(importedA, JSON.stringify(importedA)).toMatchObject({
    status: "success",
    data: { status: "imported", prepared_available: false },
  });
  ...
  const appendedA = await invoke<MainTrackClip>("main_track_append_full", {
    assetId: assetA,
  });
  ...
  expect(appendedA).toMatchObject({
    status: "success",
    revision: expect.any(Number),
  });
  expect(appendedB).toMatchObject({
    status: "success",
    revision: expect.any(Number),
  });
  ...
  const moved = await invoke<null>("main_track_move", {
    clipId: clipA,
    beforeClipId: null,
  });
  ...
  const removed = await invoke<null>("main_track_remove", {
    clipId: rightClip,
  });
  ...
  expect(
    timeline.data?.sources.map((source) => source.asset_id).sort(),
  ).toEqual([assetA, assetB].sort());
  expect(timeline.data?.clips.map((clip) => clip.source_asset_id)).toEqual([
    assetB,
    assetA,
  ]);

  await page.bringToFront();
  const playback = await page.evaluate(async ({ firstAssetId, secondAssetId }) =>
    new Promise<{ played: boolean; advanced: boolean; firstFrameReady: boolean; error: string; states: string[] }>((resolvePlayback) => {
      let played = false;
      ...
      const timeout = window.setTimeout(() => {
        unsubscribe();
        resolvePlayback({ played, advanced: false, firstFrameReady, error: "timeout", states });
      }, 10_000);
      const unsubscribe = window.doubleLove.player.onState((state) => {
        states.push(`${state.state}:${state.seconds.toFixed(3)}/${state.duration.toFixed(3)}@${state.rate.toFixed(1)}`);
        firstFrameReady ||= state.ready_for_display;
        if (state.state === "playing") {
          played = true;
          startingTime ??= state.seconds;
          if (state.seconds - startingTime > 0.1) {
            window.clearTimeout(timeout);
            unsubscribe();
            void window.doubleLove.player.pause();
            resolvePlayback({ played, advanced: true, firstFrameReady, error: "", states });
          }
        } else if (state.state === "error") {
          window.clearTimeout(timeout);
          unsubscribe();
          resolvePlayback({ played, advanced: false, firstFrameReady, error: state.error, states });
        }
      });
      void (async () => {
        await window.doubleLove.player.setBounds({ x: 20, y: 80, width: 320, height: 180 });
        await window.doubleLove.player.setPresentation({
          fit: "cover", canvasWidth: 1280, canvasHeight: 720,
          positionX: 4, positionY: -3, scale: 1.05,
          rotation: 2, opacity: 0.85, background: "#223344",
        });
        await window.doubleLove.player.loadTimeline([
          { assetId: firstAssetId, sourceStartSeconds: 0.08, sourceDurationSeconds: 0.4, outputStartSeconds: 0, outputDurationSeconds: 0.4 },
          { assetId: secondAssetId, sourceStartSeconds: 0, sourceDurationSeconds: 1, outputStartSeconds: 0.4, outputDurationSeconds: 1 },
        ], 0);
        await window.doubleLove.player.play();
      })().catch((error: unknown) => {
        window.clearTimeout(timeout);
        unsubscribe();
        resolvePlayback({ played, advanced: false, firstFrameReady, error: String(error), states });
      });
    }), { firstAssetId: assetB, secondAssetId: assetA });
  expect(playback, JSON.stringify(playback)).toMatchObject({ played: true, advanced: true, firstFrameReady: true, error: "" });
  ...
  expect(
    await invoke<string | null>("output_rate_set", { rate: "fps_30" }),
  ).toMatchObject({
    status: "success",
    revision: expect.any(Number),
    data: "fps_30",
  ...
  expect((await invoke<{ rate: string }>("timeline_get")).data?.rate).toBe(
    "fps_30",
  );
  expect(
    await invoke<string | null>("output_rate_set", { rate: null }),
  ).toMatchObject({
    status: "success",
    revision: expect.any(Number),
    data: null,
  ...
  expect(
    await invoke<Record<string, unknown>>("subtitle_style_set", {
      style: projectStyle,
    }),
  ).toMatchObject({
    status: "success",
  ...
  expect(
    (await invoke<Record<string, unknown>>("subtitle_style_get")).data
      ?.font_size,
  ).toBe(60);
  ...
  expect(
    await invoke<Record<string, unknown>>("preferences_update", {
      patch: { default_subtitle_style: defaultStyle },
    }),
  ).toMatchObject({ status: "success" });
  const applied = await invoke<Record<string, unknown>>(
  ...
  expect(mediaResponse).toEqual({
    status: 200,
    body: Array.from(expectedMedia),
  });
  const missingMediaStatus = await electronApp.evaluate(
    async ({ net }) =>
