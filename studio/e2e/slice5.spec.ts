...
function seedInstalledModels(userData: string): void {
  ...
  for (const [modelId, revision] of [
    ...
    const directory = join(modelRoot, modelId, revision);
    mkdirSync(directory, { recursive: true });
    writeFileSync(join(directory, "legacy.bin"), modelId);
  }
...
async function eventStates(): Promise<{
  progress: unknown[];
  states: Array<{ task_id: string; state: string }>;
}> {
...
test.beforeAll(async () => {
  ...
  temporaryRoot = mkdtempSync(
    join(tmpdir(), "double-love-electron-slice5-e2e-"),
  );
  ...
  await page.evaluate(() => {
    ...
    window.doubleLove.onEvent("dl://progress", (payload) =>
      captured.slice5Progress.push(payload),
    );
...
test("reveal commands open host paths without returning them to the renderer", async () => {
  const responses = (await page.evaluate(() =>
    Promise.all([
      window.doubleLove.invoke("model_reveal", { modelId: "qwen3-asr-0.6b-4bit" }),
...
test("legacy cleanup previews and removes only the current model's old versions", async () => {
  const preview = (await page.evaluate(() =>
    window.doubleLove.invoke("model_legacy_cleanup_preview", {
      modelId: "qwen3-asr-0.6b-4bit",
    }),
  )) as InvokeEnvelope<{
    removable: Array<{ model_id: string }>;
  ...
  expect(preview).toMatchObject({
    ...
    result: {
      ...
      data: {
        status: "success",
        data: {
          removable: expect.arrayContaining([
            expect.objectContaining({ model_id: "qwen3-asr-0.6b" }),
            expect.objectContaining({ model_id: "qwen3-forced-aligner-0.6b" }),
          ]),
        },
      },
    },
  ...
  const applied = (await page.evaluate(() =>
    window.doubleLove.invoke("model_legacy_cleanup_apply", {
      modelId: "qwen3-asr-0.6b-4bit",
      confirmed: true,
    }),
  ...
  expect(applied).toMatchObject({
    status: "ok",
    result: { type: "invoke", data: { status: "success" } },
  });
  const catalog = (await page.evaluate(() =>
    window.doubleLove.invoke("model_catalog", {}),
  )) as InvokeEnvelope<
    Array<{ descriptor: { id: string }; installation: { state: string } }>
  >;
  const oldAsr = catalog.result?.data?.data?.find(
    (entry) => entry.descriptor.id === "qwen3-asr-0.6b",
  );
  expect(oldAsr?.installation.state).toBe("not_installed");
});

test("model folder imports use one-time grants and never return local paths", async () => {
  const importFolder = join(temporaryRoot, "local-model-import");
  mkdirSync(importFolder, { recursive: true });
  const grant = await page.evaluate(
    async (e2ePath) =>
  ...
  const first = (await page.evaluate(
    async ({ grantToken }) =>
      window.doubleLove.invoke("model_import_folder", {
  ...
  expect(first.result?.data?.status).toBe("failed");
  expect(JSON.stringify(first)).not.toContain(importFolder);
...
test("transcribes with the test-only mock and applies a granted rough-cut export", async () => {
  ...
  expect(
    await createProjectOperation<InvokeOperation<unknown>>(page, projectRoot),
  ...
  await expect
    .poll(
      async () =>
        (await eventStates()).states.find((event) => event.task_id === taskId)
          ?.state,
    )
    .toBe("succeeded");
  ...
  const transcript = await invoke<TranscriptView>("transcript_get", {
    assetId,
  });
  ...
  const omitted = await invoke<{
    id: string;
    handles_before_ms: number;
    handles_after_ms: number;
  }>("edit_omit", { assetId, startOrdinal: 1, endOrdinal: 2 });
  ...
  expect(
    (await invoke<TranscriptView>("transcript_get", { assetId })).data?.omits,
  ).toEqual([]);
  ...
  expect(await invoke("task_cancel", { taskId: cancelTaskId })).toMatchObject({
    status: "success",
  });
  await expect
    .poll(
      async () =>
        (await eventStates()).states.find(
          (event) => event.task_id === cancelTaskId,
        )?.state,
    )
    .toBe("cancelled");
