...
async function startAndWait(name: string, payload: unknown): Promise<string> {
  ...
  await expect
    .poll(async () => {
      const states = (await capturedEvents()).filter(
        (event) => event.channel === "dl://task-state",
      );
      return states.find(
        (event) => (event.payload as { task_id?: string }).task_id === taskId,
      )?.payload;
    })
...
function containsFloatArrayShapedString(value: unknown): boolean {
  if (typeof value === "string") {
    return /\[\s*[+-]?(?:\d+\.\d*|\.\d+)(?:[eE][+-]?\d+)?(?:\s*,\s*[+-]?(?:\d+\.\d*|\.\d+)(?:[eE][+-]?\d+)?)+\s*\]/.test(
      value,
    );
  }
...
test.beforeAll(async () => {
  ...
  temporaryRoot = mkdtempSync(
    join(tmpdir(), "double-love-electron-slice6-e2e-"),
  );
  ...
  await page.evaluate(() => {
    ...
    for (const channel of [
      ...
      window.doubleLove.onEvent(channel, (payload) =>
        captured.slice6Events.push({ channel, payload }),
      );
    }
  });
  await expect
...
test("diarizes, names, merges, and keeps speaker privacy boundaries intact", async () => {
  ...
  expect(firstDiarization).toMatchObject({
    ...
    data: {
      segment_count: 1,
      speakers: [expect.objectContaining({ confirmed: false })],
    },
  });
  ...
  expect(secondDiarization).toMatchObject({
    status: "success",
  ...
  expect(
    secondDiarization.data?.merge_proposals.some(
      (proposal) =>
        new Set([proposal.left_speaker_id, proposal.right_speaker_id]).size ===
          2 &&
        [proposal.left_speaker_id, proposal.right_speaker_id].includes(
          firstSpeakerId,
        ) &&
        [proposal.left_speaker_id, proposal.right_speaker_id].includes(
          secondSpeakerId,
  ...
  const proposals = await invoke<
    Array<{ speaker_id: string; candidate_name: string }>
  >("speaker_name_proposals", { assetId: firstAssetId });
  expect(proposals.data).toContainEqual(
  ...
  const agent = await invoke<SpeakerAgentPayload>(
  ...
  const transcriptBeforeMerge = await invoke<TranscriptView>("transcript_get", {
    assetId: firstAssetId,
  });
  const speakerNames = new Map(
    speakersBeforeMerge.data?.map((speaker) => [
      speaker.id,
      speaker.display_name,
    ]),
  );
  const transcriptSpeakerNames = transcriptBeforeMerge.data?.words.flatMap(
    (word) =>
      word.speaker_assignments.map((assignment) =>
        speakerNames.get(assignment.speaker_id),
      ),
  );
  expect(transcriptSpeakerNames).toContain("李明");
  expect(
    transcriptBeforeMerge.data?.words.find((word) => word.ordinal === 4)
      ?.speaker_assignments[0]?.speaker_id,
  ...
  expect(speakersAfterMerge.data).toEqual([
    expect.objectContaining({ id: firstSpeakerId, display_name: "李明" }),
  ]);
  const transcriptAfterMerge = await invoke<TranscriptView>("transcript_get", {
    assetId: firstAssetId,
  });
  expect(
    transcriptAfterMerge.data?.words.find((word) => word.ordinal === 4)
      ?.speaker_assignments[0],
  ).toMatchObject({
    speaker_id: firstSpeakerId,
    evidence: "confirmed_speaker_merge",
  });
