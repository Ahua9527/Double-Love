...
function generateMedia(
  ...
  execFileSync(process.env.DOUBLELOVE_FFMPEG || "ffmpeg", [
    ...
    "lavfi",
    "-i",
    `color=c=${color}:s=320x180:r=${fps}:d=2`,
    "-f",
    "lavfi",
    "-i",
...
async function taskStates(): Promise<CapturedTaskState[]> {
  return page.evaluate(() => {
    const captured = window as unknown as {
      slice7TaskStates: CapturedTaskState[];
    };
...
async function transcribeAndWait(assetId: string): Promise<void> {
  ...
  await expect
    .poll(
      async () =>
        (await taskStates()).find((state) => state.task_id === taskId)?.state,
    )
    .toBe("succeeded");
}
...
function expectArtifact(
  ...
  expect(operation, JSON.stringify(operation.diagnostics)).toMatchObject({
    status: "success",
...
function cliExportXmeml(path: string): InvokeOperation<ProjectExportPreview> {
  ...
  ) as InvokeOperation<ProjectExportPreview>;
}
...
test.beforeAll(async () => {
  ...
  temporaryRoot = mkdtempSync(
    join(tmpdir(), "double-love-electron-slice7-e2e-"),
  );
...
test("delivers preview, XMEML, ASS, and a real MP4 through one-time grants", async () => {
  ...
  const firstClip = await invoke<MainTrackClip>("main_track_append_full", {
    assetId: firstAssetId,
  });
  const secondClip = await invoke<MainTrackClip>("main_track_append_full", {
  ...
  const firstTranscript = await invoke<TranscriptView>("transcript_get", {
    assetId: firstAssetId,
  });
  const secondTranscript = await invoke<TranscriptView>("transcript_get", {
    assetId: secondAssetId,
  });
  ...
  expect(preview.data?.timeline.sources).toEqual(
  ...
  expect(
    preview.data?.timeline.clips.some(
      (clip) => clip.source_asset_id === firstAssetId,
    ),
  ).toBe(true);
  expect(
    preview.data?.timeline.clips.some(
      (clip) => clip.source_asset_id === secondAssetId,
    ),
  ).toBe(true);
  expect(preview.data?.subtitle_cues.length).toBeGreaterThan(0);
  expect(
    preview.data?.compatibility.every(
      (report) => report.preserved.length > 0 && report.limitations.length > 0,
    ),
  ).toBe(true);
  expect((await invoke<number>("project_revision")).data).toBe(
    revisionBeforePreview,
  );
  expect(readdirSync(join(projectRoot, ".doublelove", "exports"))).toEqual([]);
  for (const path of [xmemlPath, assPath, mp4Path])
    expect(existsSync(path)).toBe(false);

  const xmemlGrant = await exportGrant(xmemlPath, "xml");
  const xmeml = await invoke<ProjectExportPreview>(
  ...
  const replay = await rawInvoke<ProjectExportPreview>(
    "project_export_xmeml_apply",
    {
      grantToken: xmemlGrant,
    },
  ...
  const probe = JSON.parse(
    execFileSync(
      process.env.DOUBLELOVE_FFPROBE || "ffprobe",
      [
        "-v",
        "error",
        "-select_streams",
  ...
  const history = await invoke<Array<{ operation: string }>>(
    "project_history",
    { limit: 50 },
  );
  ...
  expect(operations).toEqual(
    expect.arrayContaining(["export_xmeml", "export_ass", "export_mp4"]),
  );
  ...
  for (const expected of [
    {
      kind: "premiere_resolve_xmeml",
      path: xmemlPath,
      sha256: sha256(xmemlPath),
    },
    {
      kind: "premiere_resolve_xmeml",
      path: cliXmemlPath,
      sha256: sha256(cliXmemlPath),
    },
  ...
  const empty = await invoke<ProjectExportPreview>(
    "project_export_xmeml_apply",
    {
