...
export interface GrantPolicy {
  kind: GrantKind;
  destination: "path" | "source_path" | "target_path" | "patch.model_root";
}
...
const POLICIES: Readonly<Record<string, GrantPolicy>> = Object.freeze({
  ...
  model_import_folder: { kind: "model-import", destination: "source_path" },
...
export function applyGrantPolicy(
  ...
  if (policy.destination === "patch.model_root") {
    ...
    const declaresModelRoot =
      "model_root" in patch || "grantToken" in payload || "grantToken" in patch;
