...
export function ProjectSettings({ projectOpen, canvas, outputRate, subtitleStyle, theme, onThemeChange, onCanvasSave, onOutputRateSave, onStyleSave, history, onRestoreRevision }: ProjectSettingsProps) {
  ...
  return (
    <section className="studio-settings" aria-labelledby="settings-title">
      <header><h1 id="settings-title">项目设置</h1></header>
      ...
      <div className="studio-settings-section">
        <div><h2>自动保存记录</h2></div>
        <div className="studio-history-list">
          {history.length === 0 ? <p>还没有可恢复的编辑记录。</p> : history.map((entry) => (
            <article key={num(entry.revision)}><div><strong>{historyRecordTitle(entry.operation)}</strong><small>版本 {num(entry.revision)} · {historyTimestamp(entry.committed_at)}</small></div><button type="button" disabled={!entry.restorable} onClick={() => onRestoreRevision(num(entry.revision))}>{entry.restorable ? '恢复' : '已超出上限'}</button></article>
          ))}
        </div>
      </div>
      <div className="studio-settings-section">
        <div><h2>项目级字幕样式</h2></div>
        <div className="studio-setting-grid">
