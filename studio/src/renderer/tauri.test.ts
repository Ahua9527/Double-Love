...
describe('应用级 Tauri DTO normalize', () => {
  it('把 Rust 的 descriptor + installation 快照转成 UI 模型行', () => {
    ...
    expect(model.label).toBe('Qwen3 ASR · 0.6B')
  ...
  it('保留下载事件的进度字段，不把暂停或失败伪装成完成', () => {
    const progress = normalizeModelProgress({ model_id: 'qwen3-asr-1.7b', state: 'verifying', bytes_downloaded: 10, bytes_total: 20 })
